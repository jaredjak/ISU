...
class OwnerController {
    ...
    public String deleteOwner(@PathVariable("ownerId") int id) {
        ...
        return "Owner " + id + " deleted.";
    }

    @RequestMapping(method = RequestMethod.PUT, path = "owners/edit/phone/{id}")
    public String newPhoneNumber(@PathVariable("id") int id, @RequestBody String phone) {
        Optional<Owners> owner = ownersRepository.findById(id);
        if (owner.isPresent()) {
            Owners o = owner.get();
            o.setTelephone(phone);
            ownersRepository.deleteById(id);
            ownersRepository.save(o);
            return "Phone number changed to " + phone + ". New id: " + o.getId();
        }
