...
public class ChatServer2 {
    ...
    private static String pinnedMessage = "No pinned message";
    ...
    public void onMessage(Session session, String message) throws IOException {
        ...
        if (message.startsWith("/pin ")) {
            // Pin a message using the format "/pin <message>"
            String pinContent = message.substring(5).trim();
            setPinnedMessage(username, pinContent);
            sendMessageToPArticularUser(username, "You pinned the message: " + pinContent);
    ...
    /**
     * Pins a message in the chat, making it visible to all users.
     *
     * @param username The username of the user who pinned the message.
     * @param message  The message to be pinned.
     */
    private void setPinnedMessage(String username, String message) {
        pinnedMessage = "[Pinned by " + username + "]: " + message;
        broadcastPinnedMessage();
    }

    /**
     * Broadcasts the pinned message to all users in the chat.
     */
    private void broadcastPinnedMessage() {
        sessionUsernameMap.forEach((session, username) -> {
            try {
                session.getBasicRemote().sendText(pinnedMessage);
            } catch (IOException e) {
                logger.info("[Broadcast Pinned Exception] " + e.getMessage());
            }
        });
    }
}