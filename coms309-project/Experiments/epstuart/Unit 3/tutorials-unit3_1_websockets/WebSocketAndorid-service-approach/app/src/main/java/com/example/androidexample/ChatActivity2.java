...
public class ChatActivity2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pinnedMessageTv = findViewById(R.id.pinnedMessageTv);
        backMainBtn = (Button) findViewById(R.id.backMainBtn);
        Log.d(TAG, "onCreate: ChatActivity2 initialized.");

        // Initially hide the pinned message box
        pinnedMessageTv.setVisibility(TextView.GONE);
        ...
        // Update the BroadcastReceiver to handle pinned messages
        messageReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if ("chat2".equals(key)) {
                    ...
                    if (message.startsWith("[Pinned by")) {
                        runOnUiThread(() -> {
                            pinnedMessageTv.setText(message);
                            pinnedMessageTv.setVisibility(TextView.VISIBLE); // Show the pinned message box
                        });
                    } else {
                        runOnUiThread(() -> {
